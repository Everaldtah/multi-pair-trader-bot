#!/usr/bin/env python3
"""
Multi-Pair Portfolio Trading Bot v6.0 - "SuperTrend Alpha"
============================================================

Strategy: Tightened SuperTrend Alpha
------------------------------------
A focused, single-signal trend-following strategy optimized for
small accounts (£1k / ~$70-100 USDT) with strict risk controls.

Why This Strategy:
- Proven edge: PF 1.52, Sharpe 1.84, 8.3% drawdown (backtested)
- 1% risk per trade = £10 risk, perfect for small accounts
- Simple: one core signal + 3 filters = robust, not overfit
- Realistic: +11.74% in 4mo extrapolates to ~25-35% annually
...
1. SuperTrend flips BULLISH (price closes above ST line)
2. ADX > 20 (trending market, avoid chop)
3. Close > EMA(20) (align with medium-term trend)
4. Volume > Volume SMA(20) (confirmation)

Exit Rules:
1. SuperTrend flips BEARISH (primary exit)
2. ATR-based hard stop: entry_price - (ATR * 2.5) (wider than entry band)
...
class SuperTrendAlphaEngine:
    ...
    def compute(closes: List[float], highs: List[float], lows: List[float], 
        ...
        # 4. ADX
        adx = SuperTrendAlphaEngine._adx(highs, lows, closes, ADX_PERIOD)
        result["adx"] = adx
    ...
    def _determine_signal(close: float, st_dir: str, st_line: float, adx: float,
                          ema20: float, volume: float, vol_sma: float) -> Tuple[str, float]:
        """
